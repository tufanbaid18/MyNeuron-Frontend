...
export const ProgramList = ({ programs, isLoading }: ProgramListProps) => {
  ...
  const groupedPrograms = useMemo(() => {
    const groups = filteredByEvent.reduce((acc, program) => {
  ...
  const uniqueVenues = Array.from(new Set(programsToDisplay.map(p => p.venue).filter(Boolean)));
    ...
    <div className="w-full">
      ...
      {uniqueVenues.length > 0 ? (
        uniqueVenues.map(venue => {
          const venuePrograms = programsToDisplay.filter(p => p.venue === venue);
          return (
            <div key={venue} className="mb-10">
              <h2 className="mb-6 text-xl font-bold text-[#143B54]">Venue: {venue}</h2>
              
              <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {venuePrograms.map(program => {
                  const hasSpeaker = program.speaker_name && program.speaker_name.trim().length > 0;
                  // Handle avatar
                  ...
                  if (hasSpeaker && !program.speaker_image) {
                    ...
                    avatarSrc = getAvatarByName({ firstName: nameParts[0], lastName: nameParts.slice(1).join(" ") || undefined });
                  }

                  return (
                    <div key={program.id} className="flex flex-col rounded-2xl bg-white p-5 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] border border-gray-100">
                      {/* Top Time Pill */}
                      ...
                      {hasSpeaker && (
                        <div className="mb-6 flex items-center gap-3">
                          ...
                          <div className="flex flex-col">
                            <span className="text-sm font-bold text-gray-900">{program.speaker_name}</span>
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          ...
          {programsToDisplay.map(program => (
             <div key={program.id} className="flex flex-col rounded-2xl bg-white p-5 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] border border-gray-100">
               ...
               <h3 className="mb-2 text-lg font-bold text-[#143B54]">{program.topic}</h3>
             </div>
          ))}
        </div>
      )}
