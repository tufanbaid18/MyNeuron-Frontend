...
const GatcSpeakers = () => {
  ...
  if (error) {
    return <div className="p-8 text-red-500">Failed to load speakers.</div>;
  }
  ...
  if (!members || members.length === 0) {
    return (
      <div className="p-8">
        ...
        <Empty description="No speakers found." />
      </div>
    );
  }
      ...
      <div className="grid grid-cols-2 gap-10 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
        {members.map((member) => (
          ...
          >
            ...
            <p className="text-sm font-medium text-slate-400">Speaker</p>
          </Link>
        ))}
      </div>
    </div>
